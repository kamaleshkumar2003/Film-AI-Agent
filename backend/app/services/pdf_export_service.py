    ...
    def generate_pdf(schedule_data: dict, project_data: dict) -> bytes:
        ...
        for day in days:
            ...
            for item in day.get("items", []):
                ...
                time_range = f"{item.get('planned_start_time', '')}-{item.get('planned_end_time', '')}"
                if isinstance(sc, dict):
                    code = sc.get("scene_code") or f"SC{item.get('order_in_day', 1)}"
                    heading = f"{sc.get('scene_heading', item.get('location_name', 'Scene'))}"
                    cast_str = ", ".join(sc.get("character_names", [])) or "—"
                else:
                    code = getattr(sc, "scene_code", f"SC{item.get('order_in_day', 1)}")
                    heading = getattr(sc, "scene_heading", item.get("location_name", "Scene"))
                    chars = getattr(sc, "characters", [])
                    cast_str = ", ".join([c.name for c in chars]) if chars else "—"

                dur = f"{item.get('duration_minutes', 0)}m"
