...
async def list_schedule_versions(project_id: str, db: AsyncSession = Depends(get_db)):
    stmt = (
        ...
        .options(
            selectinload(ScheduleVersion.shooting_days).selectinload(ShootingDay.items).selectinload(ScheduleItem.scene),
...
async def get_schedule_version(project_id: str, version_id: str, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(ScheduleVersion)
        .options(
            selectinload(ScheduleVersion.shooting_days).selectinload(ShootingDay.items).selectinload(ScheduleItem.scene).selectinload(Scene.characters),
            selectinload(ScheduleVersion.conflicts)
        )
        ...
        for it in d.items:
            items_res.append({
                ...
                "scene": {
                    ...
                    "character_count": len(it.scene.characters) if it.scene else 0,
                    "prop_count": len(it.scene.props) if it.scene else 0,
                    "vehicle_count": len(it.scene.vehicles) if it.scene else 0,
                    "character_names": [c.name for c in it.scene.characters] if it.scene else []
...
'''
Path("backend/app/api/routes/schedule.py").write_text(sched_api, encoding="utf-8")

# Update backend/app/api/__init__.py
api_init = '''from fastapi import APIRouter
...
'''
Path("backend/app/api/__init__.py").write_text(api_init, encoding="utf-8")

print("Schedule API routes created and included in api_router.")
