...
weather_code = '''from abc import ABC, abstractmethod
import datetime
...
class OpenMeteoWeatherProvider(BaseWeatherProvider):
    ...
    async def get_forecast(self, latitude: float, longitude: float, target_date: datetime.date) -> Dict[str, Any]:
        ...
        url = f"https://api.open-meteo.com/v1/forecast?latitude={latitude}&longitude={longitude}&daily=temperature_2m_max,precipitation_probability_max,precipitation_sum,wind_speed_10m_max,weather_code&start_date={date_str}&end_date={date_str}&timezone=auto"
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if res.status_code == 200:
                    ...
                    return {
                        ...
                        "rain_probability": float(rain_prob or 15.0),
                        "precipitation_mm": float(precip or 0.0),
                        "wind_speed_kmh": float(wind or 12.0),
            ...
            logger.warning(f"Open-Meteo call failed: {e}, using fallback.")
...
'''
Path("backend/app/services/weather_service.py").write_text(weather_code, encoding="utf-8")
